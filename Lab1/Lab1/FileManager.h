...
public:
    static Canvas loadFromFile(std::string fileName) 
    {
        ...
        while (inFile.read(info, 6))
        {
            ...
            case 2:
            {
                figures::Triangle* newFigure = new figures::Triangle(info[1], info[2], info[3]);
                canvas.addObject(newFigure);
                if (info[5]) canvas.fillObject(canvas.getObjects().size() - 1);
                break;
            }
	...
	static void loadToFile(std::string fileName, Canvas canvas)
    {
        ...
        for (IFigure* figure : canvas.getObjects()) {
            for (uint8_t v : figure->getStartValues()) {
                outFile.write(reinterpret_cast<const char*>(&v), sizeof(uint8_t));
            }
            bool v = figure->getIsFilled();
            outFile.write(reinterpret_cast<const char*>(&v), sizeof(uint8_t));
        }
        outFile.close();
    }
